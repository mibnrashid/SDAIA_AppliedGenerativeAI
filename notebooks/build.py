...
# ====================================================================
# NOTEBOOK 0 — project_scaffold.ipynb  (12 sections)
# ====================================================================
...
"""))
    ...
    # 1 · Config
    c.append(md("## 1 · Config"))
    c.append(setup_cell(
        ["google-genai", "chromadb", "rank-bm25", "numpy", "pandas"],
        extra_imports="import numpy as np\nimport pandas as pd\nimport chromadb\nfrom rank_bm25 import BM25Okapi\nfrom datetime import datetime",
        extra_note='''
...
"""))
    ...
    c.append(breaks([
        ("`FileNotFoundError: my_docs`", "No documents uploaded yet",
         "Folder icon in the Colab sidebar → create `my_docs` → upload your files"),
        ("Everything retrieves the same chunk", "Your corpus is too small, or chunks are too large",
         "Aim for at least 20 chunks; drop `CHUNK_SIZE` to 300 and re-index"),
        ("`429` during indexing", "Free-tier embedding rate limit",
         "Raise `pause` in `embed()` to 2.0. Index once, then avoid re-running section 4"),
    ]))
...
NOTEBOOKS = {
    "project_scaffold.ipynb": scaffold,
